...
# GeneMark.hmm gene finding program <gmhmmp>; version 3.32
my $hmm = File::Spec->catfile( $gms_dir, "gmhmmp" );

# sequence parsing tool <probuild>; version 2.30
...
if ( !$par )
{
	...
	$par = GetParFileName( "virus" ) if $virus;
}
...
if ( ! $output )
{
  ...
  elsif ( $format eq "GFF3" ) { $output = File::Spec->splitpath( $seqfile ) .".gff3"; }
...
--format    <string> output coordinates of predicted genes in this format
            (default: $format; supported: LST, GFF and GFF3)
...
--virus     to run program on a eukaryotic viral genome
            (this option is the same as:  --par par_virus.default  --combine --gcode 1 --clean --fixmotif --prestart 6 --width 12 --gm  --identity 1)
