...
sub run_glimmer {
  ...
  run_program ( $o, $c ) if ! $o->{"test"};
  system("cp /annotate/databases/temp/glimmer ".$o->{"job"}."/glimmer") if $o->{"test"};
}
...
sub run_prodigal {
  ...
  run_program ( $o, $c ) if ! $o->{"test"};
  system("cp /annotate/databases/temp/prodigal ".$o->{"job"}."/prodigal") if $o->{"test"};
}
...
sub run_genemarks {
  ...
  run_program ( $o, $c ) if ! $o->{"test"};
  system("cp /annotate/databases/temp/genemarks ".$o->{"job"}."/genemarks") if $o->{"test"};
  run_program ( $o, $c );
}
...
sub run_blast {
  ...
  run_program ( $o, $c ) if ! $o->{"test"};
  system("cp /annotate/databases/temp/blast ".$o->{"job"}."/blast") if $o->{"test"};
}
...
sub run_diamond {
  ...
  run_program ( $o, $c ) if ! $o->{"test"};
  system("cp /annotate/databases/temp/diamond ".$o->{"job"}."/diamond") if $o->{"test"};
}
...
sub run_pannzer {
  ...
  run_program ( $o, $c ) if ! $o->{"test"};
  system("cp /annotate/databases/temp/pannzer ".$o->{"job"}."/pannzer") if $o->{"test"};
}
...
sub run_emapper {
  ...
  run_program ( $o, $c ) if ! $o->{"test"};
  system("cp /annotate/databases/temp/emapper ".$o->{"job"}."/emapper") if $o->{"test"};
}
...
sub parse_ab_initio {
  ...
  foreach my $l ( parse_file( $o, $o->{"job"}."/".$o->{"cds-i-program"}, "\\s+", $o->{"cds-i-program"} ) ) {
    ...
    # check and store CDS sequence feature, if successful
    if ( check_feature ( $o, $feature ) ) {
...
sub parse_homology {
  ...
  foreach my $l ( parse_file( $o, $o->{"job"}."/".$o->{"cds-h-program"}, "\\t", $o->{"cds-h-program"} ) ) {
    ...
    if ( $o->{"cds-h-program"} eq "blast" or $o->{"cds-h-program"} eq "diamond" ) {
      ...
      # check and store CDS sequence feature, if successful
      if ( check_feature ( $o, $feature ) ) {
        # gene tag, if possible, only when the feature is allowed
        if ( $l->{"gene_name"} ) {
          $feature->add_tag_value ( "gene", get_unique_gene_symbol( $o, $l->{"gene_name"} ) );
        }
        store_feature( $o, $feature );
      }
    }
...
sub parse_annotation {
    ...
    foreach my $l ( parse_file( $o, $o->{"job"}."/".$o->{"cds-a-program"}, "\\t", $o->{"cds-a-program"} ) ) {
      ...
      # product tag
      $feature->add_tag_value ( "product", $l->{"product"} );
      # remove all previous gene tags
      $feature->remove_tag( "gene" ) if $feature->has_tag("gene");
      # gene tag, if possible
      if ( $l->{"gene_name"} ) {
        $feature->add_tag_value ( "gene", get_unique_gene_symbol( $o, $l->{"gene_name"} ) );
      }
