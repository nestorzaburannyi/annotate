...
sub add_source_features {
  ...
  foreach my $seq_id ( sort keys %{$o->{"r"}} ) {
    ...
    # check and store source sequence feature, if successful
    ...
    foreach my $seq_id ( sort keys %{$o->{"r"}} ) {
        ...
        foreach my $feature ( sort { $a->start<=>$b->start || $a->end<=>$b->end } @features ) {
            ...
            # if there is no locus_tag
                ...
                while ( exists $o->{"l"}->{$new_locus_tag} ) {
...
sub add_gene_features {
  ...
  foreach my $seq_id ( sort keys %{$o->{"r"}} ) {
    foreach my $feature ( grep { $_->primary_tag ~~ ["CDS", "ncRNA", "rRNA", "tmRNA", "tRNA"] } sort { $a->start<=>$b->start || $b->end<=>$a->end } $o->{"r"}->{$seq_id}->get_SeqFeatures() ) {
      ...
      # check and store gene sequence feature, if successful
      if ( check_feature ( $o, $gene_feature ) ) {
        store_feature( $o, $gene_feature );
      }
    }
  }
}
...
sub mark_ambiguities {
  ...
  foreach my $seq_id ( sort keys %{$o->{"r"}} ) {
    ...
    while ( $seq =~ /[^ATGCN]{1,}/g ) {
      ...
      # check and store gap sequence feature, if successful
      if ( check_feature ( $o, $feature ) ) {
        store_feature( $o, $feature );
      }
    }
...
sub write_gbf_and_sqn_output {
    ...
    run_program ( $o, $o->{"cwd"}."/bin/tbl2asn/tbl2asn -i ".$o->{"job"}."/output.fsa -o ".$o->{"job"}."/output.sqn -V vb -s T -a r10k -w ".$o->{"job"}."/output.cmt" );
}
