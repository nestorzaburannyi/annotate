...
sub run_pilercr {
    ...
    system("cp /annotate/databases/temp/pilercr ".$o->{"job"}."/pilercr") if $o->{"test"};
}
...
sub run_trf {
    ...
    run_program ( $o, $c ) if ! $o->{"test"};
    system("cp /annotate/databases/temp/trf ".$o->{"job"}."/trf") if $o->{"test"};
}
...
sub parse_crispr_prediction {
  ...
  foreach my $l ( parse_file( $o, $o->{"job"}."/".$o->{"dna-c-program"}, "\\t", $o->{"dna-c-program"} ) ) {
    ...
    # check and store repeat_region sequence feature, if successful
