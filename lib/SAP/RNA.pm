...
sub run_rnammer {
    ...
    system("cp /annotate/databases/temp/rnammer ".$o->{"job"}."/rnammer") if $o->{"test"};
}
...
sub run_trnascanse {
    ...
    run_program ( $o, $c ) if ! $o->{"test"};
    system("cp /annotate/databases/temp/trnascanse ".$o->{"job"}."/trnascanse") if $o->{"test"};
}
...
sub run_aragorn {
    ...
    run_program ( $o, $c ) if ! $o->{"test"};
    system("cp /annotate/databases/temp/aragorn ".$o->{"job"}."/aragorn") if $o->{"test"};
}
...
sub run_infernal {
    ...
    run_program ( $o, $c ) if ! $o->{"test"};
    system("cp /annotate/databases/temp/infernal ".$o->{"job"}."/infernal") if $o->{"test"};
}
...
sub parse_rrna_prediction {
  ...
  foreach my $l ( parse_file( $o, $o->{"job"}."/".$o->{"rna-r-program"}, "\\s+", $o->{"rna-r-program"} ) ) {
    ...
    # check and store rRNA sequence feature, if successful
...
sub parse_trna_prediction {
  ...
  foreach my $l ( parse_file( $o, $o->{"job"}."/".$o->{"rna-t-program"}, "\\s+", $o->{"rna-t-program"} ) ) {
    ...
    # check and store tRNA sequence feature, if successful
    if ( check_feature ( $o, $feature ) ) {
      store_feature( $o, $feature );
    }
  }
...
sub parse_tmrna_prediction {
  ...
  foreach my $l ( parse_file( $o, $o->{"job"}."/".$o->{"rna-tm-program"}, "\\s+", $o->{"rna-tm-program"} ) ) {
    ...
    # check and store tmRNA sequence feature, if successful
    if ( check_feature ( $o, $feature ) ) {
      store_feature( $o, $feature );
    }
  }
...
sub parse_ncrna_prediction {
  ...
  foreach my $l ( parse_file( $o, $o->{"job"}."/".$o->{"rna-nc-program"}, "\\s+", $o->{"rna-nc-program"} ) ) {
    ...
    # check and store ncRNA/misc_feature/misc_binding sequence feature, if successful
    if ( check_feature ( $o, $feature ) ) {
      store_feature( $o, $feature );
    }
  }
